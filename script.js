...
function showQuestion() {
  ...
  const imageSource = artifact.image && artifact.image.trim()
    ? artifact.image.trim()
    : artifact.generatedImage;
  const hasCredit = Boolean(artifact.credit && artifact.credit.trim());
  if (hasCredit) {
    imageCredit.textContent = artifact.credit;
  ...
  img.onerror = () => {
    img.onerror = null;
    img.src = artifact.generatedImage;
    imageCredit.textContent = `Illustration stored in assets: ${artifact.name}`;
    imageCredit.classList.remove('hidden');
  };
  img.src = imageSource;
